---
title: 'Proyecto de Predicción de Venta de Casas en Condado King'
...
knitr::opts_chunk$set(echo = FALSE, error = TRUE, fig.align ="center", message =FALSE, warning = FALSE )
```
...
datatable(head(df, 30), callback = JS('table.page("next").draw(false);'))
```
...
Los estadísticos se presentan en la tabla 1.
Describiendo algunos estadísticos de variables importantes, en el caso de la variable respuesta, el precio, la media de este fue 540,088 dólares con una desviación de 367,127.
Si nos fijamos en algunas características importantes en la compra de un inmueble, como el número de cuartos y baños, los primeros tenian un promedio de 3.34 \~ 3 cuartos por inmueble, mientras que en el caso de los baños se contó con una media de 2.11 \~ 2 baños por inmueble.
...
Vemos si hay mlticolinealidad en las variables.
El programa nos indica que hay coeficientes que no se pueden estimar debido a un alto grado de multicolinealidad entre esas variables.
...
Podemos extraer el coeficiente de determinación del modelo anterior, el cual nos da 0.6997 o, en términos porcentuales, 69.97%, lo cual nos indica que el modelo tiene una bondad de ajuste cercana al 70%.
Es decir, las variables predictoras son responsables del 69.97% de la variación de la variable respuesta, en este caso el precio.
...
Vamos al análisis de los residuos del modelo.
Encontramos que estos no siguen una distribución normal.
Inspeccionado los residuos versus los valores predichos, vemos una clara aglomeración en el centro y un conjunto de valores predichos que se alejan de la línea de residuos=0 conforme los valores predichos crecen, lo cual no sigue un patrón aleatorio en cómo están ubicados los residuos.
En el caso del gráfico QQ-plot, estos tampoco siguen la línea recta, con un conjunto de residuos saliéndose de dicha linea.

En el gráfico de *scale- location* no se ve una línea horizontal a lo largo del gráfico, notándose que la variabilidad crece conforme los valores ajustados crecen.
En el caso del gráfico *Residuals vs. Leverage* se observa que hay valores extremos que afectan al modelo ya que muchos de ellos pasan incluso de las 10 desviaciones estándar.
...
Testeamos el grado de simetría y curtosis de los residuos así como realizamos el test de Anderson-Darling.
Obtenemos que la curtosis excede en demasía el valor de 3 asi como se aleja del grado de simetría que se quiere en una distribución normal.
Asimismo, el test de A-D rechaza la hipótesis nula de normalidad en los residuos.
...
Inspeccionamos los residuos del modelo 2.
Encontramos que, a pesar de quitar variables que consideramos que no son relevantes al modelo, los residuos siguen un patrón similar al del modelo 1.
...
Ahora eliminaremos las variables que consideremos redundantes a través de inspeccionar el nivel de correlación entre las variables predictoras y eliminar estas para proceder a estimar el modelo usando mínimos cuadrados.
Posteriormente, compararemos este modelo con el modelo anterior a fin de quedarnos con el modelo más parsimonioso y que ayude a explicar mejor el conjunto de datos.
...
Podemos corroborar que variables como `lat`, `long`.
`bedrooms`, `bathrooms`, entre otras no siguen una distribución normal.
...
Transformamos las variables para suavizar las mismas.
Ahora la variable dependiente será el logaritmo natural de `price` y procederemos a probar los resultados.
...
Evaluando el modelo 4, vemos que tanto la curtosis se acerca de mejor manera que los modelos anteriores al valor esperado de 3 y también el grado de asimetría se acerca al cero.
Sin embargo, aún se rechaza la hipótesis de normalidad en el test de Anderson-Darling.
Es decir, aún la distribución no sigue la normalidad.
...
Añadimos la interacción entre la variable `bathrooms` y el logaritmo de `sqft_living`.
Notamos una gran mejora en el coeficiente de determinación ajustado (=77.1%).
Es decir el modelo explica el 77.1% de la variabilidad de la variable dependiente.
...
Notamos una gran mejora a la hora de analizar los residuos.
Sigue notándose el patrón aleatorio de los residuos contra los valores ajutados.
La normalidad en el gráfico QQ domina a lo largo del gráfico, salvo en los extremos, lo que podría indicar que hay algunos valores extremos que siguen afectando en menor proproción la linealidad del modelo.
Asimismo, analizando el gráfico *residuals vs leverage* observamos que los residuos se aglomeran en la izquierda sin salirde del límite dado por la distanciade Cook.
Sin embargo, se puede identificar las observaciones 4025, 12778, y 15781 como los valores extremos, conocidos como *outliers*, que pueden afectar aún a los resultados del modelo.
Sin embargo, quitando estas observaciones, se puede aún mejorar el modelo.
Como se ha visto, realizando los ajustes y transformaciones necesarias, se puede llegar a un modelo que sea lo suficientemente complejo y a la vez parsimonioso para explicar los datos y las asociaciones entre ellos.
...
Dado que ya tenemos una idea de cómo se comportan los residuos, podemos hacer una inspección más detallada con el modelo que creemos que mejor nos ha dado resultado hasta el momento medido con el coeficiente de determinación en tanto que buscamos un mejor ajuste para el modelo.
Es decir el modelo 6 y ver específicamente si hay outliers y observaciones influyentes que pudieran estar afectando la distribución de los residuales.
Así, obtenemos un conjunto de resultados que nos indican que hay observaciones que sí influyen, posteriormente en los resultados del modelo.
Queda en agenda realizar modificaciones para obtener un mejor modelo.
...
Ahora comparemos los últimos 3 modelos y evaluemos el nivel de parsimoniedad entre ellos.
Es decir, un modelo que cuyo nivel de complejidad sea lo minimamente necesario para describir el conjunto de datos.
En este caso, utilizaremos la función `anova` para comparar los dos modelos y elegir al que provee el mejor ajuste y a la vez que sea el modelo más parsimonioso para describir y modelar el conjunto de datos.
...
Como primer argumento, ponemos al modelo menos complejo.
El output del análisis anova nos indica que el segundo modelo contiene una variable extra y tiene un p-value menor a 0.05, indicando que ajusta mejor a los datos para predecir el valor del precio que el modelo 5.
...
El output del análisis anova ahora nos indica que el modelo 6 performa mejor, a pesar que tiene dos variables extras, es decir es más complejo.
Sin embargo, en el análisis de comparación, se muestra que tiene un mejor ajuste que el último modelo que tiene incorporada una variable de interacción.
Así, preferimos el modelo 6 hasta el momento.
Como agenda, queda retirar variables outliers e incorporar interacciones para ver si mejora el modelo.
...
## Selección de modelos usando los métodos Best Subset Selection, Forward Stepwise Selection y Backward Stepwise Selection

```{r, include=FALSE}
#Cargamos las librerías a usar
library(leaps)
library(caret) #libreria para modelos predictivos
```
...
```{r, echo=TRUE}
dfbest <- df[,3:21]
df.bss <- regsubsets(price ~ ., data = dfbest, nvmax = 18,
                          method = "exhaustive") #le pongo 18 por el número de variables predictoras
bss <- summary(df.bss)
bss
```

#### Cp de Mallows
El Cp de Mallows nos indica que el mejor modelo contiene 17 variables predictoras
```{r}
windows()
par(mfrow=c(1,1))
plot(bss$cp, xlab = "Número de variables predictoras", ylab = "Cp",
     type = 'p')
which.min(bss$cp)
points(17, bss$cp[17], col = "red", cex = 2, pch = 20) #cex y pch son parametros para la forma del marcador
```

#### BIC
El criterio BIC nos indica que el mejor modelo contiene 15 variables predictoras BIC
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(bss$bic, xlab = "Número de variables predictoras", ylab = "BIC",
     type = 'p')
which.min(bss$bic)
points(15, bss$bic[15], col = "blue", cex = 2, pch = 20)
```

#### R2 Ajustado
El R2 ajustado nos indica que el mejor modelo contiene 17 variables predictoras
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(bss$adjr2, xlab = "Número de variables predictoras", ylab = "R2 ajustado",
     type = "p")
which.max(bss$adjr2)
points(17, bss$adjr2[17], col = "green", cex = 2, pch = 20)
```
...
Luego vamos al proceso de *validación cruzada 10* que nos indicará la performance de los modelos y posteriormente calculará las siguientes métricas de desempeño (`$R^2$`, `RMSE`, `MAE`) en cada uno de ellos.
```{r, include=FALSE, message=FALSE, warning=FALSE}
RNGkind(sample.kind = "Rejection") #define el mismo metodo de rejection como el generador
# de numeros pseudoaleatorios que permitira realizar el proceso de particion aleatorio del conjunto de datos
# siempre va la linea 49
train.cont <- trainControl(method = "cv", number = 10) #metodo de cross validation 10
```

#### Modelo con 17 variables predictoras

```{r, message=FALSE, warning=FALSE}
set.seed(3018) # Valor semilla
m1bss_17 <- train(price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + yr_renovated + zipcode + lat + long + sqft_living15 + sqft_lot15+ sqft_basement, data = dfbest, method = "lm",
             trControl = train.cont)
m1bss_17
```

#### Modelo con 15 variables predictoras

```{r, message=FALSE, warning=FALSE}
set.seed(9125) # Valor semilla
m2bss_15 <- train(price ~ bedrooms + bathrooms + sqft_living + waterfront + view + condition + grade + sqft_above + yr_renovated + zipcode + lat + long + sqft_living15 + sqft_lot15 + sqft_basement, data = dfbest, method = "lm", trControl = train.cont)
m2bss_15
```

Ya que tanto el modelo 1 y 3 utilizan las mismas variables, nos quedamos con el primero y lo compararemos con el segundo.

Comparamos las diferentes métricas de los modelos realizados:
El modelo con 15 variables presenta un menor RSME, mayor R cuadrado y un menor MAE a diferencia del modelo con 17 variables predictoras. En este caso, nos quedaríamos con el modelo de 15 variables.
...
```{r, message=FALSE, warning=FALSE}
df.fwss <- regsubsets(price ~ ., data = dfbest, nvmax = 18,
                          method = "forward")
fwss <- summary(df.fwss)
fwss
```
...
windows()
plot(fwss$cp, xlab = "Número de variables predictoras", ylab = "Cp",
     type = 'p')
which.min(fwss$cp)
points(17, fwss$cp[17], col = "black", cex = 2, pch = 20) #cex y pch son parametros para la forma del marcador
```

#### BIC
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(fwss$bic, xlab = "Número de variables predictoras", ylab = "BIC",
     type = 'p')
which.min(fwss$bic)
points(15, fwss$bic[15], col = "blue", cex = 2, pch = 20)
```

#### R2 ajustado
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(fwss$adjr2, xlab = "Número de variables predictoras", ylab = "R2 ajustado",
     type = "p")
which.max(fwss$adjr2)
points(17, fwss$adjr2[17], col = "green", cex = 2, pch = 20)
```
...
#### Modelo con 17 variables predictoras
```{r, message=FALSE, warning=FALSE}
set.seed(6713) # Valor semilla
m1fwss_17 <- train(price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + yr_built + yr_renovated + zipcode + lat + long + sqft_living15 + sqft_lot15 + sqft_basement, data = dfbest, method = "lm", trControl = train.cont)
m1fwss_17
```

#### Modelo con 15 variables predictoras
```{r, message=FALSE, warning=FALSE}
set.seed(4681) # Valor semilla
m2fwss_15 <- train(price ~ bedrooms + bathrooms + sqft_living + waterfront + view + condition + grade + yr_built+ yr_renovated+ zipcode + lat + long + sqft_living15 + sqft_lot15 + sqft_basement , data = dfbest, method = "lm", trControl = train.cont) 
m2fwss_15
```

Comparamos las diferentes métricas de los modelos realizados. El modelo con 17 variables predictoras presenta un menor RSME y MAE, así como un mayor R cuadrado. Es decir presenta unas mejores métricas que el modelo con 15 variables predictoras. Por ello, nos quedamos con el primer modelo.
```{r}
m1fwss_17$results
m2fwss_15$results 
```
...
df.bwss <- regsubsets(price ~ ., data = dfbest, nvmax = 18,
                          method = "backward")
bwss <- summary(df.bwss)
bwss
```
...
```{r, message=FALSE, warning=FALSE, fig.align='center'}
windows()
plot(bwss$cp, xlab = "Número de variables predictoras", ylab = "Cp",
     type = 'p')
which.min(bwss$cp)
points(17, bwss$cp[17], col = "black", cex = 2, pch = 20) #cex y pch son parametros para la forma del marcador
```

### BIC
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(bwss$bic, xlab = "Número de variables predictoras", ylab = "BIC",
     type = 'p')
which.min(bwss$bic)
points(15, bwss$bic[15], col = "blue", cex = 2, pch = 20)
```

### R2 ajustado
```{r, message=FALSE, warning=FALSE, fig.align='center'}
plot(bwss$adjr2, xlab = "Número de variables predictoras", ylab = "R2 ajustado",
     type = "p")
which.max(bwss$adjr2)
points(17, bwss$adjr2[17], col = "green", cex = 2, pch = 20)
```

Vemos cuáles son los coeficientes seleccionados por los criterios:
```{r, message=FALSE, warning=FALSE}
coef(df.bwss, 17)
coef(df.bwss, 15)
coef(df.bwss, 17)
...
Luego vamos al proceso de *validación cruzada 10* que nos indicará la performance de los modelos y posteriormente calculará las siguientes métricas de desempeño (R-cuadrado, RMSE y MAE) en cada uno de ellos.
```{r, include=FALSE, message=FALSE, warning=FALSE}
train.cont <- trainControl(method = "cv", number = 10)
```
...
#### Modelo con 17 variables predictoras
```{r,message=FALSE, warning=FALSE}
set.seed(6381) # Valor semilla
m1bwss_17 <- train(price ~ bedrooms + bathrooms + sqft_living + sqft_lot + floors + waterfront + view + condition + grade + sqft_above + yr_renovated + zipcode+ lat + long + sqft_living15 + sqft_lot15 + sqft_basement, data = dfbest, method = "lm", trControl = train.cont)
m1bwss_17
```

#### Modelo con 15 variables predictoras
```{r, message=FALSE, warning=FALSE}
set.seed(9284) # Valor semilla
m2bwss_15 <- train(price ~ bedrooms + bathrooms + sqft_living + waterfront + view + condition + grade + sqft_above + yr_renovated + zipcode+ lat + long + sqft_living15 + sqft_lot15 + sqft_basement, data = dfbest, method = "lm", trControl = train.cont)
m2bwss_15
```

En este caso, el modelo con 17 variables tiene un menor RSME, pero el modelo con 15 variables presenta un mayor R cuadrado, así como un menor MAE. Por lo tanto nos quedamos con el modelo de 15 variables al realizar el Backward Stepwise Selection.
```{r}
m1bwss_17$results
m2bwss_15$results
```

### Mejor modelo de los tres métodos de selección
En este caso, comparando los tres métodos de selección, el modelo que tiene mejor perfomance es el que fue seleccionado con el algoritmo *Forward Step Selection*, ya que tiene un menor RMSE, un menor MAE y posee a la vez un mayor R cuadrado. Este lo guardaremos para compararlo con el modelo que se obtiene de la regresión Lasso.
```{r}
m2bss_15$results
m1fwss_17$results
m2bwss_15$results
...
## MODELOS DE REGRESIÓN PENALIZADA: REGRESSIÓN RIDGE Y LASSO

```{r}
dfset <- df[,3:21]
x <- model.matrix(price ~ ., dfset)[ , -1]
y <- dfset$price
```
...
```{r}
library(glmnet)
dfset.ridge <- glmnet(x, y, alpha = 0)
attributes(dfset.ridge)
```

#### Valores de lambda usados para obtener diferentes modelos de regresión ridge
```{r}
dfset.ridge$lambda
```

#### Coeficientes estimados de los diferentes modelos de regresión ridge
```{r}
dfset.ridge$beta
```
...
#### Valor de lambda en el modelo 20
```{r}
dfset.ridge$lambda[20]
```

#### Coeficientes estimados obtenidos con el valor de lambda en el modelo 20
```{r}
dfset.ridge$beta[ , 20]
dfset.ridge$a0[20]
coef(dfset.ridge)[ , 20]
...
```{r}
windows()
plot(dfset.ridge, xvar = "lambda")
par(cex = 0.75)
legend("topright", legend = colnames(x), lty = c(1, 1), col = 1:8)
```

#### Para hallar el valor óptimo de lambda se usa validación cruzada
```{r}
set.seed(3018)
...
#### El modelo de regresión ridge final usando el valor óptimo de lambda
```{r}
dfset.ridge.best <- glmnet(x, y, alpha = 0, lambda = best.lambda.ridge)
coef(dfset.ridge.best)
print(coef(dfset.ridge.best),digits=22) 
...
pred.ridge <- predict(cv.out.ridge, newx = x, s="lambda.min")
...
### Regresión lasso
```{r}
dfset.lasso <- glmnet(x, y, alpha = 1)
```

#### Valores de lambda usados para obtener diferentes modelos de regresión lasso
```{r}
dfset.lasso$lambda
```

#### Coeficientes estimados de los diferentes modelos de regresión lasso
```{r, include=FALSE}
dfset.lasso$beta
```
...
#### El modelo de regresión lasso final usando el valor óptimo de lambda
```{r}
dfset.lasso.best <- glmnet(x, y, alpha = 1, lambda = best.lambda.lasso)
coef(dfset.lasso.best)
```

```{r, include=FALSE}
attributes(dfset.lasso.best)
attributes(dfset.lasso)
```
...
```{r, echo=TRUE}
anova(m1fwss_17,dfset.ridge.best)
```
...
```{r, echo=TRUE}
assess.glmnet(dfset.ridge.best,           #in this case, we are evaluating the model
              newx = x,              #in the same data used to fit the model
              newy = y )  
```

```{r, echo=TRUE}
assess.glmnet(dfset.lasso.best,           #in this case, we are evaluating the model
              newx = x,              #in the same data used to fit the model
              newy = y )  
```